...
public:
    ...
    {
        vector<int> lcp(s.size()), SA = P.back(), RSA = Reverse_Suffix_Array();
    ...
    {
        vector<int> lcp(s.size()), SA = P.back(), RSA(s.size());
        for(int i=0; i<s.size(); i++)
            RSA[SA[i]] = i;
        return RSA;
    }
...
int main()
{
    ...
    while(cin >> n >> t)
    {
        ...
        SuffixArray pq(v);
        vector <int> sa = pq.GetSuffixArray();
        vector <int> LCP = pq.LCP_Array();
        b = 0;
        for(i=0; i<=n-t; i++)
        {
            //deb(i, pq.lcp[i], pq.sa[i]);
            //deb(n, k);
            d = 1<<28;
            for(j=i; j<i+t-1; j++)
                d = min(d, LCP[j]);
