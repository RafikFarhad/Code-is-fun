...
/** Created on  : 2017-01-21-14.43 _ Saturday      */
/****************************************************/
...
public:
    int l, r, d;
    DATA(int _l=0, int _r=0, int _d=0)
    {
        l = _l;
        r = _r;
        d = _d;
    ...
    while(sf("%d %d", &n, &k)==2 and n + k)
    {
        ...
        for(i=0; i<n; i++)
        {
            for(j=i+1; j<n; j++)
            {
                if(pq.size()<k)
                {
                    pq.push(DATA(i, j, abs(ar[i]-ar[j])));
                }
                else
                {
                    if(ar[j]-ar[i]<pq.top().d)
                    {
                        pq.pop();
                        pq.push(DATA(i, j, ar[j]-ar[i]));
                    }
                    else break;
                }
            }
        }
        pf("%d %d\n", ar[pq.top().l], ar[pq.top().r]);
