...
/** Created on  : 2017-02-16-19.02 _ Thursday       */
/****************************************************/
...
ll fact[100005], m, n, inv[100005];

void PRE()
{
    int i, j;
    fact[0] = 1ll;
    inv[0] = FAST_EXP(fact[0], 6227711);
    for(i=1; i<100001; i++)
    {
        fact[i] = (fact[i-1]*1ll*i)%MOD;
        inv[i] = FAST_EXP(fact[i], 6227711);
    }
    return;
}
...
//        deb(ans);
        for(i=n, j=1; i>=1; i--, j++)
        {
            if(j&1)
            {
                ans = ans - nCr(n,j)*nPr(m-j, n-j);

            }
            else
            {
                ans = ans + nCr(n,j)*nPr(m-j, n-j);
                deb("++", nCr(n, j), "--", nPr(m-j, n-j));
                deb(m-j, n, j);
            }
            while(ans<0)
                ans+=MOD;
