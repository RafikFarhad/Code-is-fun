...
/** Created on  : 2017-01-17-21.29 _ Tuesday      */
/****************************************************/
...
public:
    int u, v;
    double w;
    DATA(int _u, int _v, double _w) :u(_u), v(_v), w(_w) {};
...
double MST(vector<DATA> p)
{
    ...
    for(i=0; i<po; i++)
    {
        ...
        {
            JOIN(x, y);
            if(CHECK_PAR(0)==CHECK_PAR(1)) return w;
        }
    }
    return -11;
}
...
//    fgets(pp, 100, stdin);
    ...
    while(takei(n)==1 and n)
    {
        ...
        for(i=0; i<n; i++)
        {
            ...
            {
                if(i==j) continue;
                x = (ax[i]-ax[j])*(ax[i]-ax[j]) + (bx[i]-bx[j])*(bx[i]-bx[j]);
                v.pb(DATA(i, j, x));
            }
        }
        sort(all(v));
