...
int main()
{
...
#endif
    ///                                    MAIN
    int t, keis=0, l1, l2, i, j, k, ar[28], br[28];
    char a[105], b[105];
    takei(t);
    getchar();
    while(t--)
    {
        ...
        l2 = strlen(gets(b));
        for(i=0; i<l1; i++)
            isalpha(a[i])&&isupper(a[i])?ar[a[i]-'A']++:ar[a[i]-'a']++;
        for(i=0; i<l2; i++)
            isalpha(b[i])&&isupper(b[i])?br[b[i]-'A']++:br[b[i]-'a']++;
        //for(i=0; i<26; i++) deb(i, ar[i], br[i]);
        for(i=0; i<26; i++)
            if(ar[i]!=br[i]) break;
        if(i==26) pf("Case %d: Yes\n", ++keis);
        else pf("Case %d: No\n", ++keis);
