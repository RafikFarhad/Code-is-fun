...
long long BIGMOD(long long base, long long power, long long mod1)
{
    if(power==0) return 1;
    long long res = BIGMOD(base, power/2, mod1);
    res = (res*res)%mod1;
    if(power & 1)
        res = (res*base)%mod1;
    return res;
...
#endif
    ///                                    MAIN
    long long *f, n, i, j, ke=0, k, l, result;
    int t;
    f = new long long [SIZE];
    f[0] = 1;
    for(i=1; i<SIZE; i++)
        f[i] = (f[i-1]*i)%mod;
    takei(t);
