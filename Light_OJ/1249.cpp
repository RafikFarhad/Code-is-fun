...
int main()
{
    ...
    while(t--)
    {
        ...
        {
            cin >> name >> a >> b >> c;
            v.pb(DATA(name, a*b*c));
        }
        sort(v.begin(), v.end());
        if(v[0].k==v[1].k) pf("Case %d: no thief\n", ++keis);
        else
        {
            if(v[n-1].k==v[n-2].k) pf("Case %d: %s took chocolate from all\n", ++keis, v[0].name.c_str());
            else pf("Case %d: %s took chocolate from %s\n", ++keis, v[0].name.c_str(), v[n-1].name.c_str());
        }
    }
    /* Coding is FUN  */
    ///                                    ENDD
