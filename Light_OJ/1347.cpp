...
int main()
{
    ...
    while(t--)
    {
        ...
        b = bb.size();
        c = cc.size();
        SuffixArray pq(aa+"{"+bb+"}"+cc);
        rsa = pq.Reverse_Suffix_Array();
        lcp = pq.LCP_Array();
        l = a+b+c+2;
        d = 0;
        for(i=2; i<l; i++)
        {
            x = Which_String(rsa[i]);
            y = Which_String(rsa[i-1]);
            z = Which_String(rsa[i-2]);
            if(x==y or y==z or z==x) continue;
            d = max(d, min(lcp[i], min(lcp[i-1], 100000000)));
        }
        csi;
        _(d);
    }
