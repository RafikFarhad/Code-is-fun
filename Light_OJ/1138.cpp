...
long long unsigned COUNT_5(long long unsigned n)
{
    long long unsigned cow=0;
    while(n)
    {
        cow += n/5;
        n/=5;
...
long long unsigned Mr_BEAN(long long unsigned n)
{
    long long unsigned hi=10e8, lo=0, ret=0, mid, p;
    mid = (lo+hi)>>1;
    //deb(mid);
    while(lo<=hi)
    {
...
#endif
    ///                                    MAIN
    long long unsigned t, keis=0, n, r;
    takellu(t);
    while(t--)
    {
        takellu(n);
        r = Mr_BEAN(n);
        if(r) pf("Case %llu: %llu\n", ++keis, r);
        else pf("Case %llu: impossible\n", ++keis);
    }
