...
//    fgets(pp, 100, stdin);
    ...
    while(t--)
    {
        ...
        {
            //deb(answer[i].x, answer[i].y, ANGLE_OF_THREE(answer[(i+1)%answer.size()], answer[i], answer[(i-1+answer.size())%answer.size()]));
            x = min(x, ANGLE_OF_THREE(answer[(i+1)%answer.size()], answer[i], answer[(i-1+answer.size())%answer.size()]));
        }
        if(answer.size()<3) x = 0.0;
        csi;
        pf("%.6lf\n", x);
    }
