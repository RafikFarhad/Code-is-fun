...
ll GO(int pos, int tot,bool equall, bool frst)
{
    if(pos==n)
    {
        return (ll)tot;
    }
    ll &ret = dp[pos][tot][equall][frst];
    if(~ret) return ret;
    ret = 0;
    int m, i;
    m = equall?(ar[pos]):9;
    ...
    for(i=0; i<=m; i++)
    {
        ret+= GO(pos+1, ((!frst)?(tot+(i==0)):0), (equall && (i==ar[pos])), (frst && i==0));
    }
    return ret;
}
...
#endif
    ///                                    MAIN
    int i, t, j, k, l, br[33],  keis=0, c, d, x, y;
    ll a, b, xx;
    ll ans1, ans2;
    takei(t);
    while(t--)
